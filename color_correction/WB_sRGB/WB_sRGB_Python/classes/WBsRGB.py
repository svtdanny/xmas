...
class WBsRGB:
  def __init__(self, gamut_mapping=2, upgraded=0):
    ...
    if upgraded == 1:
      self.features = np.load(root_path + '../models/features+.npy')  # encoded features
      self.mappingFuncs = np.load(root_path + '../models/mappingFuncs+.npy')  # correct funcs
      self.encoderWeights = np.load(root_path + '../models/encoderWeights+.npy')  # PCA matrix
      self.encoderBias = np.load(root_path + '../models/encoderBias+.npy')  # PCA bias
      self.K = 75  # K value for NN searching
    else:
      self.features = np.load(root_path + '../models/features.npy')  # encoded features
      self.mappingFuncs = np.load(root_path + '../models/mappingFuncs.npy')  # correction funcs
      self.encoderWeights = np.load(root_path + '../models/encoderWeights.npy')  # PCA matrix
      self.encoderBias = np.load(root_path + '../models/encoderBias.npy')  # PCA bias
